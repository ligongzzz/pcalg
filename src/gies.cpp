 ...
 */
RcppExport SEXP localScore(SEXP argScore,
                           SEXP argPreprocData,
                           SEXP argVertex,
                           SEXP argParents,
                           SEXP argOptions) {
    // Initialize automatic exception handling; manual one does not work any
    // more...
    BEGIN_RCPP
    ...
    double result =
        score->local(Rcpp::as<uint>(argVertex) - 1, castVertices(argParents));
    delete score;
 ...
 */
RcppExport SEXP globalScore(SEXP argScore,
                            SEXP argPreprocData,
                            SEXP argInEdges,
                            SEXP argOptions) {
    // Initialize automatic exception handling; manual one does not work any
 ...
 */
RcppExport SEXP localMLE(SEXP argScore,
                         SEXP argPreprocData,
                         SEXP argVertex,
                         SEXP argParents,
                         SEXP argOptions) {
    ...
    // Calculate local score
    std::vector<double> result = score->localMLE(Rcpp::as<uint>(argVertex) - 1,
                                                 castVertices(argParents));
    delete score;
    return Rcpp::wrap(result);
 ...
 */
RcppExport SEXP globalMLE(SEXP argScore,
                          SEXP argPreprocData,
                          SEXP argInEdges,
                          SEXP argOptions) {
    // Initialize automatic exception handling; manual one does not work any
 ...
 * 								essential graph
 * @param	argPreprocData		preprocessed data; sufficient statistic and all
 * 								parameters characterizing the score to be
 * calculated
 * @param	argAlgorithm		string indicating the causal inference algorithm
 * 								to be used. Supported options: "GIES", "GDS",
 * "DP"
 * @param	argScore			name of score object to be used. Currently
 * supported: "none" (= R object), "gauss.l0pen"
 * @param	argOptions			list of options specific for desired inference
 * algorithm
 */
RcppExport SEXP causalInference(SEXP argGraph,
                                SEXP argPreprocData,
                                SEXP argAlgorithm,
                                SEXP argScore,
                                SEXP argOptions) {
    ...
    if (!Rf_isNull(options["initialGraph"])) {
        ...
        std::cout << edges_count << " edges added" << std::endl;
    } else {
    ...
    // Cast option for fixed gaps: logical matrix, assumed to be symmetric by
    // now
    ...
    // Single phase or step of GIES
    else if (algName == "GIES-F" || algName == "GIES-B" ||
             algName == "GIES-T") {
        ...
        // Enable caching, if requested
        if (options["caching"]) graph.enableCaching();

        steps.push_back(0);
        if (algName == "GIES-F") {
            for (; steps.back() < stepLimit && graph.greedyForward(adaptive);
                 steps.back()++)
                ;
            stepNames.push_back("forward1");
        } else if (algName == "GIES-B") {
            for (; steps.back() < stepLimit && graph.greedyBackward();
                 steps.back()++)
                ;
            stepNames.push_back("backward1");
        } else if (algName == "GIES-T") {
            for (; steps.back() < stepLimit && graph.greedyTurn();
                 steps.back()++)
                ;
    ...
    // Single one or several steps of GIES into either direction
        ...
        // Steps: 3 entries, storing number of forward, backward, and turning
        // steps
    ...
    // GDS; yields a DAG, not an equivalence class!
        ...
        do {
            cont = false;
            for (uint i = 0; i < phases.size(); ++i) {
                for (steps.push_back(0); graph.greedyDAGStepDir(phases[i]);
                     steps.back()++) {
                    cont = true;
                }
                ss.str(std::string());
    ...
    // Other algorithm: throw an error
        ...
        throw std::runtime_error(algName + ": invalid algorithm name");
...
RcppExport SEXP optimalTarget(SEXP argGraph, SEXP argMaxSize) {
    ...
    for (std::vector<uint>::iterator vi = result.begin(); vi != result.end();
         ++vi)
        (*vi)++;
    return Rcpp::wrap(result);
 ...
 */
RcppExport SEXP condIndTestGauss(SEXP argVertex1,
                                 SEXP argVertex2,
                                 SEXP argCondSet,
                                 SEXP argSampleSize,
                                 SEXP argCor) {
 ...
 *
 */
RcppExport SEXP estimateSkeleton(SEXP argAdjMatrix,
                                 SEXP argSuffStat,
                                 SEXP argIndepTest,
                                 SEXP argIndepTestFn,
                                 SEXP argAlpha,
    ...
    if (indepTestName == "gauss") {
        ...
        indepTest =
            new IndepTestRFunction(&suffStat, Rcpp::Function(argIndepTestFn));
    // Invalid independence test name: throw error
    else
        throw std::runtime_error(indepTestName +
                                 ": Invalid independence test name");
...
#pragma omp parallel for
        for (int j = i + 1; j < p; j++) {
            if (adjMatrix(i, j) && !fixedMatrix(i, j)) {
                ...
                if (pMax(i, j) >= alpha) sepSet[j][i].set_size(0);
                dout.level(1)
                    << "  x = " << i << ", y = " << j
                    << ", S = () : pval = " << pMax(i, j) << std::endl;
            }
        }
    }
    for (int i = 0; i < p; i++) {
        for (int j = i + 1; j < p; j++) {
            if (fixedMatrix(i, j))
                ...
                edgeTests[0]++;
                if (pMax(i, j) < alpha) graph.addEdge(i, j);
            }
        }
    }
    ...
    graph.fitCondInd(alpha, pMax, sepSet, edgeTests,
    ...
    // Return list
    return Rcpp::List::create(Rcpp::Named("amat") = graph.getAdjacencyMatrix(),
